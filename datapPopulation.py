...
start = time.time()
netusecmd = r"net use \\10.17.172.178\QA\NimBUS-install\Probes\SystemTestingProbes interOP@123 /user:{}\administrator".format(
    gfile.hostname)
    ...
    try:
        ...
        if rc == 0:
            ...
            archive_pkg_copy_cmd = r"echo All | copy \\10.17.172.178\qa\NimBUS-install\Probes\SystemTestingProbes\{}\*.zip C:\PROGRA~1\Nimsoft\archive".format(
                gfile.uimversion)
            stdout1, stderr1, rc1 = remote_connection().run_executable("cmd.exe", arguments='''/c  {} '''.format(
                archive_pkg_copy_cmd))
...
def probe_deplyment():
    ...
    try:
        ...
        for probe in ('cdm', 'dirscan', 'logmon', 'processes', 'net_connect'):
            ...
            stdout, stderr, rc = remote_connection().run_executable("cmd.exe",
                                                                    arguments='''/c  {}'''.format(probe_deployment))
            stdout = str(stdout, 'utf-8')
            stderr = str(stderr, 'utf-8')
            if rc == 0:
...
def cfg_replacing():
    ...
    try:
        ...
        cdmcfg = r"echo All | copy \\10.17.172.178\qa\NimBUS-install\Probes\SystemTestingProbes\{}\Windows_CFG\system\cdm.cfg C:\PROGRA~1\Nimsoft\probes\system\cdm".format(
            gfile.uimversion)
        stdout, stderr, rc = remote_connection().run_executable("cmd.exe", arguments='''/c  {}'''.format(netusecmd))
        stdout, stderr, rc = remote_connection().run_executable("cmd.exe", arguments='''/c  {}'''.format(cdmcfg))
        stdout = str(stdout, 'utf-8')
        stderr = str(stderr, 'utf-8')
        if rc == 0:
            print('CDM cfg file replaced successfully...')
            dirscancfg = r"echo All | copy \\10.17.172.178\qa\NimBUS-install\Probes\SystemTestingProbes\{}\Windows_CFG\system\dirscan.cfg C:\PROGRA~1\Nimsoft\probes\system\dirscan".format(
                gfile.uimversion)
            stdout1, stderr1, rc1 = remote_connection().run_executable("cmd.exe",
                                                                       arguments='''/c  {} '''.format(dirscancfg))
            stdout1 = str(stdout1, 'utf-8')
            stderr1 = str(stderr1, 'utf-8')
            if rc1 == 0:
                print('dirscan cfg file replaced successfully...')
                logmoncfg = r"echo All | copy \\10.17.172.178\qa\NimBUS-install\Probes\SystemTestingProbes\{}\Windows_CFG\system\logmon.cfg C:\PROGRA~1\Nimsoft\probes\system\logmon".format(
                    gfile.uimversion)
                stdout2, stderr2, rc2 = remote_connection().run_executable('cmd.exe',
                ...
                if rc2 == 0:
                    ...
                    processescfg = r"echo All | copy \\10.17.172.178\qa\NimBUS-install\Probes\SystemTestingProbes\{}\Windows_CFG\system\processes.cfg C:\PROGRA~1\Nimsoft\probes\system\processes".format(
                        gfile.uimversion)
                    stdout3, stderr3, rc3 = remote_connection().run_executable('cmd.exe',
                                                                               arguments=''' /c {} '''.format(
                                                                                   processescfg))
                    ...
                    if rc3 == 0:
                        ...
                        net_connectcfg = r"echo All | copy \\10.17.172.178\qa\NimBUS-install\Probes\SystemTestingProbes\{}\Windows_CFG\network\net_connect.cfg C:\PROGRA~1\Nimsoft\probes\network\net_connect".format(
                            gfile.uimversion)
                        stdout4, stderr4, rc4 = remote_connection().run_executable('cmd.exe',
                                                                                   arguments=''' /c {} '''.format(
                                                                                       net_connectcfg))
...
def probe_restart():
    ...
    try:
        ...
        for probe in ['cdm', 'dirscan', 'logmon', 'processes', 'net_connect']:
            ...
            stdout, stderr, rc = remote_connection().run_executable("cmd.exe",
                                                                    arguments='''/c  {}'''.format(probe_deactivate))
            stdout = str(stdout, 'utf-8')
            stderr = str(stderr, 'utf-8')
            if rc == 0:
                ...
                stdout, stderr, rc = remote_connection().run_executable("cmd.exe",
                                                                        arguments='''/c  {}'''.format(probe_activate))
                stdout = str(stdout, 'utf-8')
                stderr = str(stderr, 'utf-8')
                if rc == 0:
